...
Select a.CensusDate
	, Count(a.PatientID) as CensusForSurgeryFromOR
Into #SurgeryCensusORPatients
FROM [ADTCMart].[ADTC].[vwCensusFact] a
	Inner Join (Select Distinct PatientID, AccountNum 
				From [ORMart].[dbo].[vwRegionalORCompletedCase]) b 
...
From #tempCensusCounter a
	Left join (Select CensusForSurgery, count(CensusDate) as SurgeryCensusForAllPatients 
			   From #SurgeryCensusAllPatients Group by CensusForSurgery) b 
		on a.CensusCount = b.CensusForSurgery
	Left join (Select CensusForSurgeryFromOR, count(CensusDate) as SurgeryCensusForORPatients 
			   From #SurgeryCensusORPatients Group by CensusForSurgeryFromOR) c 
	    on a.CensusCount = c.CensusForSurgeryFromOR
Group by a.CensusCount
	, b.SurgeryCensusForAllPatients
	, c.SurgeryCensusForORPatients
